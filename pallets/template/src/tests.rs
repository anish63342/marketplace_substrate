...
fn it_stores_correctly() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(TemplateModule::sell_product(Origin::signed(1),CF4865oY,iphone12,80000,Anish C));
		// Read pallet storage and assert an expected result.
		assert_eq!(TemplateModule::proof(), Some(CF4865oY,iphone12));

		assert_ok!(TemplateModule::sell_product(Origin::signed(1),HG9666kJ,apple macbook air,94000,AnishCY));
		assert_eq!(TemplateModule::proof(), Some(HG9666kJ,apple macbook air));

		assert_ok!(TemplateModule::sell_product(Origin::signed(1),BD5400eL,nike airmax11,14000,Shinichi K));
